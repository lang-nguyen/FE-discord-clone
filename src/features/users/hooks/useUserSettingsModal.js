...
export function useUserSettingsModal({ open, onOpenChange }) {
  ...
  const navItemClass = (section) =>
    `mt-2 flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm transition-colors ${
      activeSection === section
        ? "bg-hover-bg font-semibold text-primary-text"
        : "text-muted-text hover:bg-hover-bg/50 hover:text-primary-text"
    }`;
