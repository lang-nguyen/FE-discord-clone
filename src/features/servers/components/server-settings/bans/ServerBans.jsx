...
import { useBans } from "@/features/servers/hooks/useBans";
...
export const ServerBans = ({ serverName }) => {
  ...
  return (
    <div className="flex-1 min-w-0">
      ...
      <div className="mb-4">
        <div className="relative w-full rounded bg-server-sidebar-bg flex items-center px-2">
          <input
            type="text"
            className="w-full bg-transparent text-primary-text text-sm py-2 px-2 outline-none placeholder:text-muted-text/70"
      ...
      <div className="space-y-1">
        {bansLogic.bans.length === 0 ? (
          <div className="text-center py-10 text-muted-text text-sm">
            No banned users found.
          </div>
        ) : (
          bansLogic.bans.map(ban => (
            <div key={ban.id} className="flex items-center justify-between p-2 rounded hover:bg-hover-bg group">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-primary-text font-medium">{ban.name}</span>
                    <span className="text-muted-text text-sm ml-1">@{ban.username}</span>
                  </div>
                  {ban.reason && (
                    <div className="text-muted-text text-xs mt-0.5 max-w-md truncate">
                      Reason: {ban.reason}
                    </div>
                  )}
                </div>
      ...
      {bansLogic.toastMessage && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 bg-user-panel-bg border border-[#1e1f22] text-white px-4 py-3 rounded-[4px] shadow-lg flex items-center gap-3 z-[100] animate-in fade-in slide-in-from-top-4 duration-300">
