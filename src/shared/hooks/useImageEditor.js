...
export function useImageEditor({ open, imageSrc }) {
  ...
  if (vW > 0 && vH > 0 && box.w > 0 && box.h > 0) {
    ...
    const targetW = box.w * IMAGE_EDITOR_CONFIG.COVER_RATIO;
    const targetH = box.h * IMAGE_EDITOR_CONFIG.COVER_RATIO;
    ...
    cropSize = Math.floor(Math.min(baseUI_W, baseUI_H) * IMAGE_EDITOR_CONFIG.CROP_RATIO);
  }
